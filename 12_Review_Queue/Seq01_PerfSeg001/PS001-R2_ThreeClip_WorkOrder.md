...
- **RENDERED 2026-07-01** (Veo std, first+last, 8 s native each; Dossiers `PS001R2-C1/C2/C3.md`): **C1** ✅ directed witness move (⚠️ mid ghosting artifact — trim/re-roll); **C2** ✅ snail motif + eyes shift, no smile (⚠️ glint reads pronounced — Director judges glint vs beam); **C3** ⚠️ **end-frame defect** — interpolates to un-retrofitted HERO-03 → Jen reverts fuller + temple bg + non-snail. **BLOCKER: retrofit HERO-03 v5 (slim Jen + snail + sandy shoreline) then re-render C3.** Then Gemini GVR → editorial assembly to 12.6 s → EP lock.
