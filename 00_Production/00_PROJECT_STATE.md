...
- **THE GATE NOW (EP's open call):** (a) **re-engineer** — exercise Veo **reference-image conditioning on the locked CCA face sheets** (identity lever, not yet tried) + a face-revealing camera + **Director (Sam) revisits the emotion / shot structure** (emotion is Sam's lane); or (b) **stand up a proper motion MCP** (Render Bridge as a first-class tool) before further iteration. *(Grok not a gate — Creative Motion Consultant.)*
